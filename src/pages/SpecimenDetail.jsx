...
function SpecimenHero({ specimen, rarity, onBack }) {
  return (
    <>
      {/* Back nav */}
...
function SpecimenIdentity({ specimen, rarity }) {
  return (
    <div>
...
function AiConfidenceBanner({ conf, confColor, confLabel, purity }) {
...
function SpecimenStatsRow({ purity, collectionScore, evoLevel, rarityColor }) {
  return (
    <div className="grid grid-cols-3 gap-2">
      ...
      <StatBlock
        icon={<Star size={14} />}
        label="Score"
        value={collectionScore ? `${collectionScore}/5` : '—'}
        color={rarityColor}
      />
      <StatBlock
...
function EvolutionTracker({ evoLevel, evoStage, rarity }) {
  return (
...
function SpecimenTabs({ activeTab, setActiveTab, specimen, rarity, purity, evoLevel, collectionScore }) {
  const tabs = [
...
export default function SpecimenDetail() {
  ...
  return (
    <div className="min-h-screen pb-32" style={{ background: 'hsl(240 20% 4%)' }}>
      <SpecimenHero specimen={specimen} rarity={rarity} onBack={() => navigate(-1)} />

      {/* Main content */}
      <div className="px-4 -mt-6 max-w-md mx-auto space-y-4">
        <SpecimenIdentity specimen={specimen} rarity={rarity} />

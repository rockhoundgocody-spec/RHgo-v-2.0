...
export function calculateCatchOutcome(spawn, throwCount, roll = Math.random()) {
  ...
  if (spawn.is_shiny) {
    return roll < Math.min(catchChance + 0.2, 0.95) ? "shiny" : "escape";
  } else if (roll < catchChance * 0.15) {
    return "critical";
  } else if (roll < catchChance) {
    return "success";
  } else {
    return throwCount >= 2 ? "success" : "escape"; // mercy on 3rd throw
  }
}
...
export function useARCamera(videoRef) {
  useEffect(() => {
    let stream;
...
export function EncounterTopBar({ spawn, theme, onDismiss }) {
  return (
...
export function EncounterResultCard({ phase, result, theme, spawn, xp, onDismiss, onTryAgain }) {
...
export function useEncounterState({ spawn, onCatch, theme, reduceMotion }) {
  ...
  const handleThrow = useCallback(async () => {
    ...
    const outcome = calculateCatchOutcome(spawn, throwCount);

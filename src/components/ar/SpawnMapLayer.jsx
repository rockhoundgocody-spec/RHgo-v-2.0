...
export default function SpawnMapLayer({ spawns = [], onSpawnTap, caughtToday, dailyCap }) {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-[800]">
      {/* Daily cap banner */}
      {cappedOut && (
        <div className="absolute top-28 left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider"
          style={{ background: 'hsla(0,60%,20%,0.9)', border: '1px solid hsla(0,80%,50%,0.4)', color: 'hsl(0,80%,70%)' }}>
          Daily cap reached — returns tomorrow
        </div>
      )}

      {/* Spawn pins — centered on map, offset from middle by rarity */}
      {spawns.map((spawn, i) => {
        const rarityIdx = RARITY_ORDER.indexOf(spawn.rarity);
        // Scatter around center in a circle pattern
        const angle = (i / spawns.length) * 2 * Math.PI;
        const radius = 28 + rarityIdx * 8; // rarer = slightly wider ring
        const x = 50 + Math.cos(angle) * radius;
        const y = 48 + Math.sin(angle) * radius;
        const isHovered = hovered === spawn.id;
        ...
        return (
          ...
          >
            ...
            <motion.div
              ...
              style={{
                width: spawn.rarity === 'legendary' ? 46 : spawn.rarity === 'rare' ? 40 : 34,
                height: spawn.rarity === 'legendary' ? 46 : spawn.rarity === 'rare' ? 40 : 34,
            ...
            >
              ...
              {spawn.is_shiny && (
                <div className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full flex items-center justify-center"
                  style={{ background: 'hsl(45,100%,55%)', fontSize: 8 }}>✨</div>
              )}
            </motion.div>
      ...
      {/* Heat ring — community activity visual around center */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none rounded-full"
        style={{ width: 280, height: 280, border: '1px solid hsla(280,80%,60%,0.12)', background: 'radial-gradient(circle, hsla(280,80%,60%,0.04) 0%, transparent 70%)' }} />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none rounded-full"
        style={{ width: 180, height: 180, border: '1px solid hsla(195,100%,60%,0.1)', animation: 'badge-pulse-glow 4s ease-in-out infinite' }} />
    </div>
  );
}
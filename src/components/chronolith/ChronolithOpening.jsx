...
export function PointOfLight({ phase, imageUrl }) {
  return (
    <AnimatePresence>
      {phase >= 1 && phase < 3 && (
        <motion.div
...
export function OpeningText({ phase }) {
  return (
    <AnimatePresence>
      {phase >= 2 && phase < 3 && (
        <motion.div
...
export function HypothesesList({ hypotheses = [], leadingH }) {
...
export function InvestigationSummary({ phase, caseData, imageUrl, hypotheses, leadingH }) {
  return (
    <AnimatePresence>
      {phase >= 3 && (
        <motion.div
        ...
        >
          ...
          <HypothesesList hypotheses={hypotheses} leadingH={leadingH} />
        </motion.div>
      )}
    </AnimatePresence>
  );
...
export function EnterTrialButton({ phase, onEnter }) {
  return (
    <AnimatePresence>
      {phase >= 4 && (
        <motion.button
...
export default function ChronolithOpening({ caseData, imageUrl, onEnter, onSkip }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col items-center justify-center overflow-hidden"
      ...
      <PointOfLight phase={phase} imageUrl={imageUrl} />
      <OpeningText phase={phase} />
      <InvestigationSummary phase={phase} caseData={caseData} imageUrl={imageUrl} hypotheses={hypotheses} leadingH={leadingH} />
      <EnterTrialButton phase={phase} onEnter={onEnter} />
    </div>
  );
}

...
export default function CrystalNav({ onTabClick, pathname }) {
      ...
      {tabs.map((tab) => {
        ...
        return (
          <button
            key={tab.to}
            onClick={() => onTabClick(tab.to, isActive)}
            className="relative flex flex-col items-center gap-1 px-3 py-2 rounded-full transition-colors select-none min-w-[52px] min-h-[48px] justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-hud-cyan/80"
            style={{
  ...
  return (
    <div className="flex flex-col items-center gap-1 -mt-6 px-1 select-none">
      <motion.button
        onClick={onClick}
        whileTap={{ scale: 0.92 }}
        className="relative flex items-center justify-center cursor-pointer rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amethyst-glow/80"
        style={{
          width: 58,
          height: 58,
          borderRadius: '50%',

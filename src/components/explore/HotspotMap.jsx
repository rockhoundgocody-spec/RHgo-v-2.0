...
// ── Icon Instance Caches ───────────────────────────────────────────────────────
// Instantiating L.divIcon on every render causes reference inequality on <Marker icon={...}> props,
// forcing React-Leaflet to execute marker.setIcon() and recreate DOM nodes for every marker on the map.
// Caching L.divIcon instances in module-level Map objects keyed by visual property combinations preserves
// reference equality and prevents unnecessary DOM thrashing on re-renders.
...
export function makeHotspotIcon({ color, isActive, isGlowing, hasGap, difficulty, highContrast }) {
  const cacheKey = `${color}_${Boolean(isActive)}_${Boolean(isGlowing)}_${Boolean(hasGap)}_${difficulty}_${Boolean(highContrast)}`;
...
export function makeSpecimenIcon(rarity, highContrast) {
  const cacheKey = `${rarity}_${Boolean(highContrast)}`;

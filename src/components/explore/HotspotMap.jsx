...
// Module-level icon caches to avoid recreating Leaflet L.divIcon instances on every render.
// Sharing cached icon references prevents React-Leaflet from calling marker.setIcon() and thrashing DOM nodes.
...
function makeHotspotIcon({ color, isActive, isGlowing, hasGap, difficulty, highContrast }) {
  const cacheKey = `${color}_${isActive}_${isGlowing}_${hasGap}_${difficulty}_${highContrast}`;
...
function makeSpecimenIcon(rarity, highContrast) {
  const cacheKey = `${rarity}_${highContrast}`;

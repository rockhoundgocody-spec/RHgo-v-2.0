          ...
          >
            ...
            <div className="overflow-y-auto px-5 pb-8" style={{ maxHeight: 'calc(85vh - 20px)' }}>
              <div className="flex items-center justify-between mb-5">
                <h2 id="create-capsule-title" className="text-lg font-black text-white">
                  Log a Trip
                </h2>
                <button
                  onClick={onClose}
                  aria-label="Close trip log sheet"
                  className="w-8 h-8 rounded-full flex items-center justify-center transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50"
              ...
              <div className="space-y-4">
                <Field label="Trip Name" htmlId={nameId}>
                  <input
                    id={nameId}
                    type="text"
                    ...
                    maxLength={80}
                    className="w-full px-3 py-2.5 rounded-xl text-white text-sm outline-none transition focus-visible:ring-2 focus-visible:ring-amethyst-glow/60"
                    style={{ background: 'hsla(255,30%,12%,0.7)', border: '1px solid hsla(280,80%,65%,0.3)' }}
                  />
                </Field>

                <Field label="Location" htmlId={locationId}>
                  <div className="relative">
                    ...
                    <input
                      ...
                      maxLength={80}
                      className="w-full pl-8 pr-3 py-2.5 rounded-xl text-white text-sm outline-none transition focus-visible:ring-2 focus-visible:ring-amethyst-glow/60"
                      style={{ background: 'hsla(255,30%,12%,0.7)', border: '1px solid hsla(280,80%,65%,0.3)' }}
                    />
                  </div>
                </Field>

                <Field label="Date" htmlId={dateId}>
                  <div className="relative">
                ...
                <Field label="Mood">
                  <div className="flex flex-wrap gap-2" role="group" aria-label="Select mood snapshot">
                    {MOODS.map((m) => (
                      <button
                        key={m.value}
                        type="button"
                      ...
                      >
                        <span aria-hidden="true" className="mr-1">{m.icon}</span>
                        {m.label}
                      </button>
                    ))}
                  </div>
                ...
                <Field label="Story (optional)" htmlId={storyId}>
                  <textarea
                    id={storyId}
                    value={form.story}
                    ...
                    maxLength={2000}
                    rows={3}
                    className="w-full px-3 py-2.5 rounded-xl text-white text-sm outline-none resize-none transition focus-visible:ring-2 focus-visible:ring-amethyst-glow/60"
                    style={{ background: 'hsla(255,30%,12%,0.7)', border: '1px solid hsla(280,80%,65%,0.3)' }}
                  />
                </Field>

                {error && (
                  <p className="text-rose-400 text-xs text-center" role="alert">{error}</p>
                )}
                ...
                <button
                  ...
                  aria-disabled={saving || !form.expedition_name.trim()}
                  className="w-full py-3.5 rounded-2xl font-black text-white text-sm transition active:scale-95 disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amethyst-glow/60"
...
function Field({ label, htmlId, children }) {
  return (
    <div>
      {htmlId ? (
        <label htmlFor={htmlId} className="block text-[10px] uppercase tracking-[0.2em] text-white/40 mb-1.5 font-semibold">
          {label}
        </label>
      ) : (
        <div className="text-[10px] uppercase tracking-[0.2em] text-white/40 mb-1.5 font-semibold">
          {label}
        </div>
      )}
      {children}

...
export default function ExportBar({ logs }) {
  ...
  const getCsvAriaLabel = () => {
    if (!hasLogs) return 'CSV export disabled — no specimens in log';
    if (exporting === 'csv') return 'Exporting CSV spreadsheet…';
    if (done === 'csv') return 'CSV spreadsheet exported successfully';
    return `Export ${count} log ${count === 1 ? 'entry' : 'entries'} as CSV spreadsheet`;
  };

  const getKmzAriaLabel = () => {
    if (!hasLogs) return 'KMZ export disabled — no specimens in log';
    if (exporting === 'kmz') return 'Exporting KMZ file…';
    if (done === 'kmz') return 'KMZ file exported successfully';
    return `Export ${count} log ${count === 1 ? 'entry' : 'entries'} as KMZ for Google Earth`;
  };

  const tooltipTitle = hasLogs
    ? (format) => `Export ${count} log ${count === 1 ? 'entry' : 'entries'} as ${format.toUpperCase()}`
    : () => 'No specimens in log to export';

...
## 2026-07-12 - Theme-Aware HUD Interactive Button Polish
**Learning:** Combining Framer Motion micro-interactions (e.g., scale tap) with custom keyframe animations often strips default browser focus outlines, rendering keyboard navigation completely blind. Applying custom theme-aware ring colors (`focus-visible:ring-[color]/50`) integrated with native accessibility properties (`aria-pressed` or `aria-expanded`) preserves interactive feedback and delights screen reader users.
**Action:** For highly interactive/custom buttons, always pair focus rings with the element's local color scheme (amethyst, emerald, amber, cyan) and supply standard ARIA states (`aria-pressed`, `aria-expanded`) to ensure screen reader and keyboard synchronization.
